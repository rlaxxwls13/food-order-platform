...
public class AiLogServiceTest {
    ...
    void updateAiDescription_success() {
        ...
        UpdateAiDescriptionCommand command =
                new UpdateAiDescriptionCommand(aiLogId, "수정된 설명");

        AiDescription aiDescription = mock(AiDescription.class);
        Product product = mock(Product.class);
        Store store = mock(Store.class);

        when(aiDescriptionRepository.findById(aiLogId)).thenReturn(Optional.of(aiDescription));
        when(aiDescription.getProductId()).thenReturn(productId);
        when(aiDescription.getResponseText()).thenReturn("기존 설명");
        ...
        when(aiDescription.getId()).thenReturn(aiLogId);
        when(aiDescription.getRequestText()).thenReturn("원본 설명");
        when(aiDescription.getUpdatedAt()).thenReturn(LocalDateTime.now());

        // when
        UpdateAiDescriptionResult result =
                aiLogService.updateAiDescription(command, userId, Role.OWNER);

        // then
        verify(product).changeDescription("수정된 설명");
        verify(aiDescription).changeDescription("수정된 설명");

        assertThat(result.getAiLogId()).isEqualTo(aiLogId);
        verify(product).changeDescription("수정된 설명");
        verify(aiDescription).changeDescription("수정된 설명");
    }
    ...
    void deleteAiDescription_success() {
        ...
        when(aiDescription.getId()).thenReturn(aiLogId);
        when(aiDescription.getDeletedBy()).thenReturn(userId);
        when(aiDescription.getDeletedAt()).thenReturn(LocalDateTime.now());

        // when
        DeleteAiDescriptionLogResult result =
                aiLogService.deleteAiDescription(aiLogId, userId, Role.OWNER);

        // then
        verify(aiDescription).softDelete(userId);

        assertThat(result.getAiLogId()).isEqualTo(aiLogId);
        assertThat(result.getDeletedBy()).isEqualTo(userId);
    }

    @DisplayName("AI 로그 삭제 실패 - 로그 없음")
    @Test
    void deleteAiDescription_fail_log_not_found() {
        UUID aiLogId = UUID.randomUUID();

        when(aiDescriptionRepository.findById(aiLogId))
                .thenReturn(Optional.empty());

        assertThatThrownBy(() ->
                aiLogService.deleteAiDescription(aiLogId, 1L, Role.OWNER))
                .isInstanceOf(BusinessException.class)
                .satisfies(ex -> {
                    BusinessException be = (BusinessException) ex;
                    assertThat(be.getErrorCode()).isEqualTo(ErrorCode.NOT_EXISTED_AI_LOG);
                });
    }
    ...
    void deleteAiDescription_fail_no_permission() {
        ...
        when(storeRepository.findById(storeId)).thenReturn(Optional.of(store));
        when(store.getOwnerId()).thenReturn(otherOwner);
